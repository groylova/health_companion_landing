...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'bmrCalculator.metadata' });
...
export default async function BmrCalculatorPage({ params }: Props) {
  ...
  const t = await getTranslations('bmrCalculator');
        ...
        <Container>
          ...
          <div className="grid items-start gap-10 md:grid-cols-2 md:gap-12">
            ...
            <div className="-mx-2 w-auto min-w-0 md:mx-0 md:w-full">
              <CalorieCalculator mode="bmr" messagesNamespace="bmrCalculator" />
            </div>
          </div>
        </Container>
      </section>
        ...
        <Container>
          <div className="mx-auto max-w-3xl space-y-12">
            ...
            <ContentSection title={t('seo.h2_how')}>
              <p>{t('seo.how_p1')}</p>
              <p>{t('seo.how_p2')}</p>
              {/* Rich-text via t.rich — the <tdee> / <deficit> markers in the
            ...
            <div className="my-12 overflow-hidden rounded-2xl border border-slate-200 bg-white/70 p-8 shadow-soft">
              <div className="mx-auto flex max-w-2xl flex-col items-center text-center">
                ...
                <div className="mt-6">
                  <AppStoreBadge buttonLocation="bmr_calculator_conversion" />
                </div>
              </div>
            </div>
            ...
            <RelatedGuides slugs={['tdeeCalculator', 'calculator', 'foodDiary', 'stayConsistent']} />
          </div>
        </Container>
      </article>
